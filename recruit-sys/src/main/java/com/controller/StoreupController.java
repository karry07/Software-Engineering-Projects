...
public class StoreupController {
    ...
    public R check(@RequestBody Map<String, Object> params){
        ...
        Long refid = Long.valueOf(params.get("refid").toString());

        // 在数据库中查找对应的收藏记录
        Long storeupId = storeupService.checkIfStored(userid, refid);
//        System.out.println(storeupId);

        // 如果找到了记录，返回收藏的ID，否则返回一个表示未找到的消息
        if (storeupId != null) {
            return R.ok().put("data", storeupId);
        } else {
            //什么也不返回
//            return R.ok();
            return null;
    ...
    public R deleteById(@PathVariable("id") Long id){
        boolean result = storeupService.deleteById(id);
        if (result) {
            return R.ok("删除成功");
        } else {
            return R.error("删除失败");
