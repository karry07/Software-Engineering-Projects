      ...
      <el-row>
        <el-col :span="24">
          <el-form-item v-if="type!='info'" label="关联表id" prop="refid">
            <el-input v-model="ruleForm.refid" placeholder="关联表id" clearable></el-input>
          </el-form-item>
          <div v-else>
            <el-form-item v-if="ruleForm.refid" label="关联表id" prop="refid">
              <el-input v-model="ruleForm.refid" placeholder="关联表id" readonly></el-input>
            </el-form-item>
          </div>
        </el-col>
        <el-col :span="24">
          <el-form-item v-if="type!='info'" label="评论内容" prop="content">
            <el-input v-model="ruleForm.content" placeholder="评论内容" clearable></el-input>
          </el-form-item>
          <div v-else>
            <el-form-item v-if="ruleForm.content" label="评论内容" prop="content">
              <el-input v-model="ruleForm.content" placeholder="评论内容" readonly></el-input>
            </el-form-item>
          </div>
        </el-col>
        <el-col :span="24">
          ...
          <div v-else>
            <el-form-item v-if="ruleForm.userid" label="用户id" prop="userid">
              <el-input v-model="ruleForm.userid" placeholder="用户id" readonly></el-input>
            </el-form-item>
          </div>
        </el-col>
      </el-row>
  ...
  methods: {
    ...
    onSubmit() {
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `job_post_comments/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.job_post_commentsCrossAddOrUpdateFlag = false;
                  this.parent.search();
                }
              });
            } else {
...
.main-content {
  background-image: url('../../../assets/img/commentbackground.png'); /* 背景图片 */
  ...
  animation: custom-animation 2s ease-in-out; /* 动画 */
}
