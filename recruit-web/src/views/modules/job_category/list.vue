      ...
      <div class="table-content">
        <el-table
          ...
          <el-table-column
              ...
              label="操作">
            <template slot-scope="scope">
              <el-button v-if="isAuth('job_category','修改')" type="primary" icon="el-icon-edit"
                         @click="addOrUpdateHandler(scope.row.id)" circle style="background-color: #409EFF;">修改
              </el-button>
              <el-button v-if="isAuth('job_category','删除')" type="primary" icon="el-icon-delete"  circle
                         @click="deleteHandler(scope.row.id)" style="background-color: #F56C6C;">删除
              </el-button>
            </template>
          </el-table-column>
        </el-table>
...
.main-content {
  background-image: url('../../../assets/img/jobcategorybackground.png'); /* 确保路径正确 */
